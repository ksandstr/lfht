...
/* multithreaded push20/pop15 test on the non-blocking single-link list. */
...
static void *bash_items_fn(void *param_ptr)
{
	...
	for(int i=0; i < TEST_SIZE; i++) {
		for(int j=0; j < 20; j++) {
			push(list, thread_id * TEST_SIZE * 20 + 20 * i + j);
		}
		for(int j=0; j < 15; j++) {
			struct item *it = pop(list);
			darray_push(*ret, it);	/* also pushes NULLs */
		}
	}
...
int main(void)
{
	...
	for(size_t i=0; i < N_THREADS; i++) {
		int n = pthread_create(&oth[i], NULL, &bash_items_fn, list);
		if(n != 0) {
