 ...
 * items are present, while using an exceedingly bad hash function.
 */
...
int main(void)
{
	...
	for(int i=0; i < 10000; i++) {
		...
		if(!ok && adds_ok) {
			diag("couldn't add `%s'", s);
			adds_ok = false;
		}
		if(adds_ok && found_immed && !str_in(&ht, s)) {
	...
	ok(adds_ok, "test strings were added");
