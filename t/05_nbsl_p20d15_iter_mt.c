...
/* tests on iteration under multithreaded push20/del15 load. */
...
static size_t presence_mask(
	struct nbsl *list, struct item **them, size_t n_them)
{
...
static void *p20_d15_with_iters_fn(void *param_ptr)
{
	...
	for(int i=0; i < TEST_SIZE; i++) {
		...
		unsigned found = presence_mask(list, them, 20);
		...
		/* check that the deleted ones have actually gone away. */
		found = presence_mask(list, them, 20);
...
int main(void)
{
	...
	for(size_t i=0; i < N_THREADS; i++) {
		int n = pthread_create(&oth[i], NULL, &p20_d15_with_iters_fn, list);
		if(n != 0) {
